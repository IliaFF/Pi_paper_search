...
## 6. Починить `hypa: command not found`, если нужно

Пакет `@hypabolic/pi-hypa` ставит бинарник в `~/.pi/agent/npm/node_modules/.bin/hypa`, но этот путь не всегда попадает в `PATH`. Если в shell/Pi видите `hypa: command not found`, выполните:

```bash
bash scripts/fix_hypa_path.sh
```

Скрипт создаст symlink:

```text
~/.local/bin/hypa -> ~/.pi/agent/npm/node_modules/.bin/hypa
```
...
## 7. Установить локальный academic MCP
...
## 9. Опциональные ключи для академических источников
...
## 13. Быстрый smoke test после переноса
